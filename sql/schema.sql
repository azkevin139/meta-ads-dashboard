...
CREATE TABLE accounts (
  ...
  last_leads_sync_scan_count INTEGER DEFAULT 0,
  last_leads_sync_ad_count INTEGER DEFAULT 0,
  last_leads_sync_mode TEXT DEFAULT 'incremental',
...
CREATE TABLE identity_collision_groups (
  id BIGSERIAL PRIMARY KEY,
  account_id INTEGER NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,
...
CREATE TABLE identity_collision_members (
  id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE identity_collision_resolutions (
  id BIGSERIAL PRIMARY KEY,
  collision_group_id BIGINT NOT NULL REFERENCES identity_collision_groups(id) ON DELETE CASCADE,
...
CREATE INDEX idx_identity_collision_groups_account_status ON identity_collision_groups(account_id, status, updated_at DESC);
CREATE INDEX idx_identity_collision_groups_key ON identity_collision_groups(account_id, collision_key);
...
CREATE INDEX idx_csp_violation_reports_created ON csp_violation_reports(created_at DESC);
CREATE INDEX idx_csp_violation_reports_directive ON csp_violation_reports(effective_directive, created_at DESC);
CREATE INDEX idx_csp_violation_reports_source ON csp_violation_reports(source_file, blocked_uri, created_at DESC);
